...
VM_NAME="${VM_NAME:-routeros}"
VM_MEMORY="${VM_MEMORY:-128M}"
VM_DISK_SIZE="${VM_DISK_SIZE:-128M}"
...
# Copy to runtime file 
mkdir -p routeros-instance
[ ! -f /routeros-instance/routeros.img ] && cp $ROUTEROS_IMAGE /routeros-instance/routeros.img

# Resize VM
qemu-img resize /routeros-instance/routeros.img "${VM_DISK_SIZE}"
...
exec qemu-system-x86_64 \
   ...
   "$@" \
   -drive format=raw,file=/routeros-instance/routeros.img
